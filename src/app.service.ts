...
export class AppService {
  ...
  async searchAndBroadcastRates(currencyRequired: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const message =
          'El precio actual de la moneda publicado en el BCV que buscas es: ';

          console.log("la moneda buscada",currencyRequired)
        ...
        if (isRecoverable) {
          ...
          if (attempt < this.maxRetries) {
            ...
            console.log(
              `Máximo de intentos alcanzado (${this.maxRetries}). Retornando error.`,
            );
          ...
          resultMessage.error = 1;
          console.log(
            `Error crítico detectado. No se reintentará. Estado: 500.`,
          );
  ...
  private getChange(html: string): number {
    try {
      ...
      throw new HttpException(
        'La tasa no pudo ser obtenida, posiblemente hubo un cambio en el sitio del BCV, contactar al desarrollador (2)',
        500,
      );
    }
  }
